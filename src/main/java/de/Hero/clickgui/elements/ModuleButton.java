...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (ManateePlus.getInstance().settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : ManateePlus.getInstance().settingsManager.getSettingsByMod(imod)) {
					if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				ManateePlus.getInstance().clickGui.closeAllSettings();
				this.extended = b;
