...
public class HoleESP extends Module {
    ...
    public void setup(){
        ...
        ManateePlus.getInstance().settingsManager.rSetting(rangeS);
        ...
        ManateePlus.getInstance().settingsManager.rSetting(rainbow = new Setting("heRainbow", this, false));
        ManateePlus.getInstance().settingsManager.rSetting(mode = new Setting("heMode", this, "Full", modes));
        ManateePlus.getInstance().settingsManager.rSetting(width = new Setting("heLineWidth", this, 3, 1, 10, true));
    }
    ...
    private void drawBox(BlockPos blockPos, int r, int g, int b) {
        ...
        if(mode.getValString().equalsIgnoreCase("Outline")){
            ManateePlusTessellator.drawBoundingBox(bb, width.getValInt(), color.getRGB());
        } else if (mode.getValString().equalsIgnoreCase("Bottom")) {
            ManateePlusTessellator.drawBox(blockPos, color.getRGB(), GeometryMasks.Quad.DOWN);
        } else if(mode.getValString().equalsIgnoreCase("OutlineBottom")){
            ManateePlusTessellator.drawBoundingBoxBottom(bb, width.getValInt(), color.getRGB());
        } else {
            ManateePlusTessellator.drawBox(blockPos, color.getRGB(), GeometryMasks.Quad.ALL);
        }
    }
